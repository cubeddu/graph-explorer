...
const useSPARQL = (connection: ConnectionConfig) => {
  ...
  }, [_sparqlFetch, connection.graphDbUrl]);
  ...
  const fetchNeighborsFunc = useCallback(async (req, options) => {
    ...
    const bNode = _blankNodes.get(req.vertexId);
    if (bNode?.neighbors) {
      return storedBlankNodeNeighborsRequest(_blankNodes, request);
    }

    const response = await fetchNeighbors(_sparqlFetch(options), request) as NeighborsResponse;
    const vertices = replaceBlankNodeFromNeighbors(_blankNodes, request, response);
    return { vertices, edges: response.edges };
  }, [_sparqlFetch, _blankNodes]);
  ...
  const keywordSearchFunc = useCallback(async (req, options) => {
    ...
    const vertices = replaceBlankNodeFromSearch(_blankNodes, reqParams, response);
